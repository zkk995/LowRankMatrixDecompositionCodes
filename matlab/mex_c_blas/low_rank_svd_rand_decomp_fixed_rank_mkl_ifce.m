% interface to mkl based mex file
...
function [U,S,V] = low_rank_svd_rand_decomp_fixed_rank_mkl_ifce(A,k,p,vnum,q,s)
    ...
    % delete @zkk995@gmail.com   % warm up
    % B = randn(10,10);
    % start_mkl_mex(B);         
    % pause(0.2);

    % call mex file
    [U,S,V] = low_rank_svd_rand_decomp_fixed_rank_mkl_mex(A,k,p,vnum,q,s);    
end
