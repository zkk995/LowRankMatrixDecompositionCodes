...
void low_rank_svd_decomp_fixed_rank_or_prec(mat *M, int64_t k, double TOL, int64_t *frank, mat **U, mat **S, mat **V){
    int64_t i,m,n,r,rankMode,tolMode;
    double sval;
    mat *Mc, *Uf, *Sf, *Vtf, *Vf;
...
int64_t vnum, int64_t kstep, int64_t q, int64_t s, int64_t *frank, mat **U, mat **S, mat **V){
    int64_t i,j,m,n,l,nstep;
    double val;
    m = M->nrows; n = M->ncols;
    mat *Q, *B, *BBt;
    mat *Bt, *Qhat, *Rhat, *Uhat, *Vhat_trans;

    int64_t rankMode,tolMode;
    double elapsed_secs;
...
void randomized_low_rank_svd4(mat *M, int64_t kstep, int64_t nstep, int64_t p, mat **U, mat **S, mat **V){
    int64_t i,j,m,n,knew;
    double val;
    m = M->nrows; n = M->ncols;
    mat *Q, *B, *BBt;
...
void pivotedQR_mkl(mat *M, mat **Q, mat **R, vec **I){
    int64_t i,j,k,m,n,Rrows,Rcols,Qrows,Qcols;
    mat *Mwork;
    vec *col_vec;
    m = M->nrows; n = M->ncols;
    k = min(m,n);
    ...
    *I = vector_new(n);
    for(i=0; i<n; i++){
        vector_set_element(*I,i,(int64_t)(Iarr[i]-1));
    }
...
void pivoted_QR_of_specified_rank(mat *M, int64_t k, int64_t *frank, mat **Qk, mat **Rk, vec **I){
    int64_t i,j,ind,q,m,n,max_colnorm_index,column_norm_zero,loop_lim,checkQRresult;
    double max_colnorm,tempval,val1,val2,val3;
    int64_t * max_colnorm_indices; 
...
outputs: matrices Q and B s.t. M \approx Q*B 
*/
void randQB_pb(mat *M, int64_t kstep, int64_t nstep, int64_t p, int64_t s, mat **Q, mat **B){
    int64_t i,j,m,n,l,step;
    double dotp,elapsed_time;
    int64_t *inds_local, *inds_global;
...
void id_two_sided_rand_decomp_fixed_rank(mat *M, int64_t k, int64_t p, int64_t q, int64_t s, vec **Icol, vec **Irow, mat **T, mat **S){
    int64_t m,n,frank;
    mat *MI, *MIt;
    m = M->nrows;
    n = M->ncols;
...
void id_two_sided_blockrand_decomp_fixed_rank_or_prec(mat *M, int64_t k, int64_t p, double TOL, int64_t kstep, int64_t q, int64_t s, int64_t *frank, vec **Icol, vec **Irow, mat **T, mat **S){
    int64_t m,n,nstep = ceil(k/kstep);
    mat *MI, *MIt;
